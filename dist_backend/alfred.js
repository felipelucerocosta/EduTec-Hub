...
const conexion_pg_1 = __importDefault(require("./conexion_pg")); // 👈 IMPORTAMOS LA CONEXIÓN A POSTGRESQL
const bcrypt = __importStar(require("bcrypt")); // 👈 IMPORTAMOS BCRYPT
const nodemailer = __importStar(require("nodemailer")); // <- cambiado desde "import nodemailer from 'nodemailer'"
const router = (0, express_1.Router)();
// Middleware: permitir acceso a usuarios autenticados (admin, profesor, alumno)
function requireAuth(req, res, next) {
    const rol = req.session?.usuario?.rol;
    if (!rol)
        return res.status(401).json({ error: 'Unauthorized', message: 'Inicia sesión para usar Alfred.' });
    const allowed = ['admin', 'profesor', 'alumno'];
    if (allowed.includes(String(rol)))
        return next();
...
async function sendVerificationEmail(to, code) {
    try {
        ...
        console.error('sendVerificationEmail error details:', err && err.response || err.message || err);
        throw err;
    }
}
// ===================================
...
// ===================================
router.post('/ask-alfred', requireAuth, async (req, res) => {
    const { chatHistory } = req.body;
    if (!chatHistory) {
        return res.status(400).json({ error: 'El historial del chat es requerido.' });
    }
    const apiKey = process.env.GEMINI_API_KEY || "AIzaSyCvttzZon067xS7DnQsgQIgNXvaMFTEpBw";
    if (apiKey === "" || !apiKey) {
...
router.post('/generate-password', async (req, res) => {
    ...
    try {
        ...
        const userQuery = await conexion_pg_1.default.query('SELECT id_usuario FROM usuarios WHERE correo = $1', [email]);
        ...
        if (isInstitutionalEmail(email)) {
            ...
            await conexion_pg_1.default.query(`INSERT INTO email_verifications (user_id, code, expires_at, used)
                 VALUES ($1, $2, NOW() + INTERVAL '10 minutes', false)`, [userId, code]);
            try {
            ...
            await conexion_pg_1.default.query('UPDATE usuarios SET contrasena = $1 WHERE id_usuario = $2', [hashedPassword, userId]);
            return res.status(200).json({
...
router.post('/verify-email-code', async (req, res) => {
    ...
    try {
        ...
        const userId = userQuery.rows[0].id_usuario;
        const vq = await conexion_pg_1.default.query(`SELECT id, expires_at, used FROM email_verifications
        ...
        try {
            ...
            await conexion_pg_1.default.query('UPDATE email_verifications SET used = true WHERE id = $1', [row.id]);
            return res.status(200).json({
                success: true,
                password: newPassword,
...
// =======================
async function callGeminiWithRetry(apiKey, payload, maxAttempts = 3) {
    ...
    while (attempt < maxAttempts) {
        attempt++;
        try {
            const resp = await axios_1.default.post(apiUrl, payload, { headers });
            return resp;
        }
        catch (err) {
            lastErr = err;
