...
router.get('/clases/:claseId/trabajos', async (req, res) => {
    ...
    `;
      ...
      `;
            const grupalRes = await conexion_pg_1.default.query(grupalQ, [userId]);
            const grupalMap = new Map(grupalRes.rows.map((e) => [e.trabajo_id, e]));
...
router.get('/trabajos/:id', async (req, res) => {
    ...
    `;
        ...
        if (req.session?.usuario?.rol === 'alumno') {
          ...
          `;
                    const membersRes = await conexion_pg_1.default.query(membersQ, [entrega.id]);
        ...
        `;
                const grupalRes = await conexion_pg_1.default.query(grupalQ, [userId, id]);
...
router.get('/clases/:claseId/alumnos', async (req, res) => {
    const { claseId } = req.params;
    const userId = getUserId(req);
    if (!userId) {
        return res.status(401).json({ error: 'No autorizado' });
    ...
    try {
        ...
        const claseQ = await conexion_pg_1.default.query('SELECT codigo FROM clases WHERE id = $1', [claseId]);
    ...
    `;
        const result = await conexion_pg_1.default.query(alumnosQ, [codigo]);
        return res.json(result.rows);
    }
    catch (error) {
