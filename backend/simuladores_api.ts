...
function getKeyword(materia: string): string {
...
// ─────────────────────────────────────────────────────────────
router.get('/simuladores/:claseId', async (req: Request, res: Response) => {
  const { claseId } = req.params;
  const userId = getUserId(req);
  if (!userId) return res.status(401).json({ error: 'No autorizado' });

  try {
    const claseRes = await pool.query('SELECT materia, nombre FROM clases WHERE id = $1', [claseId]);
    ...
    const simDB = await pool.query(
      'SELECT * FROM simuladores WHERE (clase_id = $1 OR materia_keyword = $2) AND activo = 1',
      [claseId, keyword]
    );
...
// ─────────────────────────────────────────────────────────────
router.post('/simuladores/sesion', async (req: Request, res: Response) => {
  const userId = getUserId(req);
  if (!userId) return res.status(401).json({ error: 'No autorizado' });

  const { simulador_tipo, clase_id, datos_sesion, duracion_segundos } = req.body;
  if (!simulador_tipo) return res.status(400).json({ error: 'Tipo de simulador requerido' });

  try {
    // Buscar o crear el simulador en DB
    let simId: number;
    const simExistRes = await pool.query(
      'SELECT id FROM simuladores WHERE tipo = $1 AND (clase_id = $2 OR clase_id IS NULL) LIMIT 1',
      [simulador_tipo, clase_id || null]
    );
    ...
    if (simExistRes.rows.length > 0) {
      ...
      const claseRes = clase_id
        ? await pool.query('SELECT materia FROM clases WHERE id = $1', [clase_id])
        : { rows: [{ materia: 'general' }] };
      const materia = claseRes.rows[0]?.materia || 'general';
      const keyword = getKeyword(materia);

      const insertRes = await pool.query(
        `INSERT INTO simuladores (materia_keyword, nombre, tipo, clase_id) VALUES ($1, $2, $3, $4) RETURNING id`,
        [keyword, simulador_tipo, simulador_tipo, clase_id || null]
      );
      simId = insertRes.rows[0]?.id || insertRes.insertId;
    }

    await pool.query(
      `INSERT INTO sesiones_simulador (simulador_id, usuario_id, datos_sesion, completado, duracion_segundos) VALUES ($1, $2, $3, 1, $4)`,
      [simId, userId, datos_sesion ? JSON.stringify(datos_sesion) : null, duracion_segundos || null]
    );
...
// ─────────────────────────────────────────────────────────────
router.get('/simuladores/mis-sesiones', async (req: Request, res: Response) => {
  const userId = getUserId(req);
  if (!userId) return res.status(401).json({ error: 'No autorizado' });
